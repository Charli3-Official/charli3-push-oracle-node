...
poetry install
```
Next, you will need to create a config.yml file containing the necessary configuration settings. You can use the provided [config.example.yml](config.example.yml) as a starting point.
...
Refer to the [config.example.yml](config.example.yml) file for an example configuration.
